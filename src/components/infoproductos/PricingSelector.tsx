'use client'

import { ReactNode, useState } from 'react'
import Link from 'next/link'

type PlanType = 'monthly' | 'yearly'

type PricingSelectorProps = {
  children?: ReactNode
}

export default function PricingSelector({ children }: PricingSelectorProps) {
  const [selectedPlan, setSelectedPlan] = useState<PlanType>('yearly') // Default to yearly (recommended)

  return (
    <div className="space-y-6">
      <div className="grid sm:grid-cols-2 gap-4 mb-2">
        {/* Monthly Plan */}
        <div
          onClick={() => setSelectedPlan('monthly')}
          className={`border-2 rounded-2xl p-4 lg:p-5 text-center transition-all duration-300 cursor-pointer relative ${
            selectedPlan === 'monthly'
              ? 'border-teal-500 bg-teal-50/10 shadow-[0_0_25px_rgba(20,184,166,0.2)] scale-102'
              : 'border-gray-300 hover:border-teal-400'
          }`}
        >
          {selectedPlan === 'monthly' && (
            <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
              <span className="bg-teal-500 text-white px-3 py-1 rounded-full text-xs font-bold">
                SELECCIONADO
              </span>
            </div>
          )}
          <div className="text-sm font-bold text-gray-500 mb-2 uppercase">Mensual</div>
          <div className="text-2xl lg:text-3xl font-bold text-slate-900 mb-2">$11.99</div>
          <div className="text-sm text-gray-600">/monthly</div>
          <div className="text-xs text-gray-500 mt-3">
            $143.88 si pagas mes a mes por 12 meses
          </div>
        </div>

        {/* Annual Plan - Recommended */}
        <div
          onClick={() => setSelectedPlan('yearly')}
          className={`border-2 rounded-2xl p-4 lg:p-5 text-center bg-orange-50/20 relative transition-all duration-300 cursor-pointer ${
            selectedPlan === 'yearly'
              ? 'border-orange-500 bg-orange-50/50 shadow-[0_0_30px_rgba(249,115,22,0.2)] scale-102'
              : 'border-gray-300 hover:border-orange-400'
          }`}
        >
          <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 z-10">
            <span className="bg-orange-500 text-white px-3 py-1 rounded-full text-xs font-bold">
              {selectedPlan === 'yearly' ? 'SELECCIONADO' : 'AHORRA 47%'}
            </span>
          </div>
          <div className="text-sm font-bold text-orange-600 mb-2 uppercase">Anual</div>
          <div className="text-2xl lg:text-3xl font-bold text-slate-900 mb-2">$97</div>
          <div className="text-sm text-gray-600">/yearly</div>
          <div className="text-xs text-green-600 font-bold mt-3">
            Ahorra $46.88 frente al pago mensual anualizado
          </div>
        </div>
      </div>

      {children}

      {/* Direct Link to Checkout */}
      <Link
        href={
          selectedPlan === 'monthly'
            ? 'https://buy.stripe.com/4gMdRag3n0Ms0ABcHI8IU01'
            : 'https://buy.stripe.com/7sY00keZjbr6cjj6jk8IU02'
        }
        target="_blank"
        rel="noopener noreferrer"
        className="block w-full bg-gradient-to-r from-orange-500 to-orange-600 text-white text-center px-6 py-4 rounded-xl font-bold text-base hover:from-orange-600 hover:to-orange-700 transition shadow-lg hover:shadow-orange-500/20"
      >
        🚀 Obtener Acceso a la App
      </Link>
    </div>
  )
}
